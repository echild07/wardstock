...
`../hosted/README.md`. Confirm `API_SYNC_TOKEN` in
`../hosted/config/config.php` — you'll need its exact value in step 4.

### 4. Secrets file
Copy `wardstock_secrets.json.example` (in this folder) to `/share/wardstock_secrets.json`
...
and `nodered/system_test_flow.json` (five separate imports). Deeper
per-endpoint GoDaddy API check (secrets file + every `api/*.php` URL,
read-only / empty-POST probes): import `nodered/godaddy_api_test.json`
the same way — click **Run GoDaddy API Test**, read the debug sidebar.
Tokens print as length + last 4 only.
...
- `nodered/godaddy_backup_flow.json` — daily (3am) + manual, pulls the
  FULL current incidents/Daily Log/Therapy/Medications/dosage-history
  from GoDaddy and mirrors it into a new dedicated local SQLite database,
...
- `nodered/incident_digest_flow.json` — 21:00 (HA clock) + manual. GETs
  ...
  had incidents (`occurred_at`). Copy `HA/share/incident_digest.py`
  to `/share/wardstock_incident_digest.py`. SMTP keys in
  `wardstock_secrets.json`. Companion: `incident_digest_test.json` (`--dry-run`).
- `nodered/godaddy_restore_flow.json` — manual only, the highest-stakes
  ...
  wiped GoDaddy database. `PLAN.md` §22. Read its own tab `info` before
  running it for real, same as `wherewhen_data_restore_flow.json`. No
  companion test flow, same reasoning as Oura Backfill (this manual flow
  already is its own diagnostic tool).
- `nodered/godaddy_restore_from_file_flow.json` — manual only, sibling to
  the one above but reads an arbitrary WardStock-export-shaped JSON file
  (`/share/lucius_restore_import.json`) instead of HA's local SQLite —
  for restoring from an external backup (e.g. a converted mysqldump) when
  there's no existing SQLite mirror to restore from. `PLAN.md` §22. Same
  "read the tab info first" caution. No companion test flow, same
  reasoning as the flow above.
...
- `nodered/godaddy_api_test.json` — companion test for every flow that
  calls GoDaddy `api/*.php`. Reads `/share/wardstock_secrets.json`, prints
  the resolved `https://aileeward.com/wardstock/...` URLs, then live-calls
  the GET endpoints plus empty-JSON POSTs that the PHP rejects with 400
  (no row written). Skips `pull_manual_data.php` on purpose (that GET
  updates `last_ha_pull_at`).
...
**`nodered/all_flows_merged.json` — every flow above, concatenated into one importable file (Ward, Aug 2026).** Each flow file is just a JSON array of nodes with its own `tab` entry, so all twenty (five required + fifteen optional) can be merged into one array and imported in a single Menu → Import, rather than one at a time — confirmed no node-id collisions across the full set, **except** three shared SQLite config nodes, each appearing once per flow file that uses it *on purpose*: `cfg_lucius_status_db` (11 identical copies — every flow that writes job status), `cfg_lucius_godaddy_backup_db` (2 identical copies — `godaddy_backup_flow.json`/`godaddy_restore_flow.json`, PLAN.md §22), and `cfg_sqlite_scratch` (2 identical copies — `sqlite_test_flow.json`/`godaddy_backup_test.json`, both throwaway-table tests sharing one scratch file). Node-RED should deduplicate identical-id config nodes on import rather than creating redundant database connections; not yet confirmed against a real import of the full merged file. Meant for a from-scratch or delete-and-reload setup; for updating just the flows that actually changed in a given batch, use the individual files instead. **Regenerated by Claude whenever any flow changes** — if a flow file and this one ever look out of sync, the individual file is the source of truth; ask for `all_flows_merged.json` to be rebuilt.
...
- Confirm the `file`/`file in` nodes' paths (`/share/wardstock_secrets.json`,
...
- Deploy `../hosted/mysql/upgrade_from_2.0.0.sql` first (safe
...
twenty flow JSON files (one exception: `godaddy_backup_test.json`'s core SQLite-serialization logic was actually run for real against a genuine SQLite engine outside of Node-RED — see `PLAN.md` §22 — everything else here is still reasoned-not-confirmed). Specific things most likely to need real fixing:
