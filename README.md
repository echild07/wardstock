...
- **`homeassistant/`** — a background sync and analytics engine running on Home Assistant OS (Node-RED + InfluxDB). Pulls high-fidelity Oura data WardStock itself never captured, pushes summary data up to WardStock, pulls manually-entered data back down for disaster-recovery backup, and reports sync status back to WardStock. Core sync flows built and system-tested end to end; a body composition import feature is planned but not yet built — see `homeassistant/PLAN.md` for the full design and `homeassistant/README.md` for setup.
...
**Current version: 2.1.2 "Lucius"** — project-wide, covering both pieces. Major version bumped from 1.x ("Sidroh") specifically to mark this expansion from a single GoDaddy app into a two-piece project. See `godaddy/README.md`'s Versioning section for the full `Major.SQL.Code` scheme (currently only tracked/applied on the GoDaddy piece, since the HA piece doesn't have a database of its own to version yet).
