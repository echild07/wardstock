...
Nothing from this setup goes into the add-on's own config beyond whatever the real Configuration tab turns out to need. The four real connection values live in `/share/lucius_secrets.json` on the Node-RED side (see `lucius_secrets.json.example` in this folder):
