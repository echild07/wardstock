...
**`nodered/all_flows_merged.json` — every flow above, concatenated into one importable file (Ward, Aug 2026).** Each flow file is just a JSON array of nodes with its own `tab` entry, so all fourteen (five required + nine optional) can be merged into one array and imported in a single Menu → Import, rather than one at a time — confirmed no node-id or tab-id collisions across the full set. Meant for a from-scratch or delete-and-reload setup; for updating just the flows that actually changed in a given batch, use the individual files instead (re-importing an already-configured flow risks either a duplicate tab or losing its hand-set `api-call-service` **Server** field, which is never in the exported JSON to begin with). **Regenerated by Claude whenever any flow changes** — if a flow file and this one ever look out of sync, the individual file is the source of truth; ask for `all_flows_merged.json` to be rebuilt.
